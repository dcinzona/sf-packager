...
exports.buildPackageDir = function (dirName, name, metadata, packgeXML, destructive, cb) {
	...
	mkdirp(packageDir, (err) => {
		...
		if (err) {
			return logger.error('Failed to write package directory ' + packageDir);
		}

		fs.writeFile(packageDir + packageFileName, packgeXML, 'utf8', (err) => {
			if (err) {
				return ('Failed to write xml file: ' + err);
			}
...
exports.copyFiles = function (sourceDir, buildDir, files, cb) {
		...
		if (file) {
			...
			if (file.endsWith('-meta.xml')) {
				...
				try {
					...
					logger.log('%s does not exist', file + '-meta.xml');
					metaExists = false;
				}
