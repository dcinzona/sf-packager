...
var _log = function(text){
    ...
    _vorpal ? _vorpal.log(text) : console.log(text);
    return text;
}
...
var verbose = function(message, header){
    if(logLevel > 1) this.log(message, header);
}

var debug = function(message, header){
    if(logLevel == 2){
        !header ? _log(color.x230('[DEBUG] ') + parseMessage(message)) : 
...
var parseMessage = function(message){
  return typeof message == typeof {} ? '\n' + cardinal.highlight(beautify(message, null, 2, 80)) : message;
}
