...
var sign = function(username, clientid, istest, certFile){
  ...
  if(certFile){
    ...
    logger.error(certFile + ' was not found.');
    return { error: certFile + ' was not found.' };
  }
  return jwt.sign(claims, 'secret-didnotprovidecert');
}

var getToken = function(username, clientid, istest, certFile, cb){
  var client = new Client();
  var loginurl = istest ? 'https://test.salesforce.com' : 'https://login.salesforce.com';
  var tokenUrl = loginurl + '/services/oauth2/token';
  var assertion = sign(username, clientid, istest, certFile);
  if(assertion.error){
    return;
  }
  var args = {
